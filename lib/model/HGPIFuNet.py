...
class HGPIFuNet(BasePIFuNet):
    ...
    def filter(self, images):
        ...
        model_path = './OV_model/FP16//HGFilter.xml'
        device_name = 'CPU'

        core = Core()
        model = core.read_model(model_path)
        input_tensor = images.cpu().numpy()
        print("Debug Log:", input_tensor.shape)
        # Infer
        compiled_model = core.compile_model(model, device_name)
        st = timeit.default_timer()
        results = compiled_model.infer_new_request({0: input_tensor})
        print('OpenVINO: ', timeit.default_timer() - st)
        ...
        print('CUDA : ', timeit.default_timer() - st)
    ...
    def query(self, points, calibs, transforms=None, labels=None):
        ...
        model_path = './OV_model/FP16/SurfaceClassifier.xml'
        device_name = 'CPU'
    ...
    def weight2ONNX(self):
        print("start to convert ...")
        onnx_path = "HGFilter.onnx"
...
class HGPIFuNetVINO(BasePIFuNet):
    '''
    HG PIFu network uses Hourglass stacks as the image filter.
    It does the following:
        1. Compute image feature stacks and store it in self.im_feat_list
    ...
    def __init__(self,
        ...
        #  load model from open vino
        # self.image_filter = None
        self.image_filter = HGFilter(opt)

        # load model from open vino
        # self.surface_classifier = None
        self.surface_classifier = SurfaceClassifier(
            filter_channels=self.opt.mlp_dim,
            num_views=self.opt.num_views,
            no_residual=self.opt.no_residual,
